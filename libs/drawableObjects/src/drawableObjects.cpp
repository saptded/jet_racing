...
//    }
//}
DrawableArc::DrawableArc(Point start, Point end, Point center){
...
//    float radius = calcRadius(element._start, element._center);
//    if (radius != calcRadius(element._end, element._center)){
//        std::cout << "wrong arc radius = " << radius << std::endl;
//        return;
//    }
//    float angleStart = getAngle(element._center, element._end); //поменято сознательно тк испраили направление
//    float angleEnd = getAngle(element._center, element._start);
//    if (angleStart < angleEnd){
//        angleStart += 2*M_PI;
//    }
//    std::cout << "arc " << element._center.x << ", " << element._center.y << " : angles st, e : ";
//    std::cout << angleStart/M_PI*180 << " " << angleEnd/M_PI*180 << std::endl;
//    int vertexCount = (int)(radius);
//    float angleStep = (angleEnd-angleStart)/(float)(vertexCount-1);
//    std::cout << " angleStep " << angleStep << std::endl;
...
//        arcs[j+1] = sf::VertexArray (sf::LineStrip, vertexCount);
//        for (int i = 0; i < vertexCount; i++){
//            arcs[j+1][i].position = sf::Vector2f(element._center.x-(radius+j*0.75)*cosf(angle), element._center.y-(radius+j*0.75)*sinf(angle));
//            arcs[j+1][i].color = color;
//            angle+=angleStep;
//        }
//    }
