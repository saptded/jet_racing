        ...
        for (const auto &it_blocks_tree : stage_tree) {
            ...
            for (const auto &it_elements_tree : block_tree) {
                ...
                if (figure == "propeller") {
                    ...
                    std::shared_ptr<Propeller> propeller_pointer(new Propeller(start, end, center, isDynamic, std::make_shared<DrawablePropeller>(start, end, center)));
                    block.elements.push_back(propeller_pointer);
                }

                if (figure == "accelerator") {
                    std::shared_ptr<Accelerator> accelerator_pointer(new Accelerator(start, end, center, std::make_shared<DrawableAccelerator>(start, end, center)));
