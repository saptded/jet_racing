...
void RacerController::changeRotationSpeed(const Command &_rotation, Racer &racer,
                                          const float &extraAccelerate) const {  // SpeedX - left; SpeedY - right
    switch (_rotation) {
        case left:
    ...
    if (!stop) {
        if (std::abs(racer._speed.speedX) > std::abs(_maxSpeed + std::copysignf(extraAccelerateX, _maxSpeed) * 4)) {
            if (racer._speed.speedX > _maxSpeed + extraAccelerateX * 4) {
                racer._speed.speedX -= OVER_SPEED_DECEL;
            }
            if (racer._speed.speedX < (-_maxSpeed - extraAccelerateX * 4)) {
                racer._speed.speedX += OVER_SPEED_DECEL;
            }
        ...
        if (std::abs(racer._speed.speedY) > std::abs(_maxSpeed + std::copysignf(extraAccelerateY, _maxSpeed) * 4)) {
            if (racer._speed.speedY > _maxSpeed + extraAccelerateY * 4) {
                racer._speed.speedY -= OVER_SPEED_DECEL;
            }
            if (racer._speed.speedY < (-_maxSpeed - extraAccelerateY * 4)) {
                racer._speed.speedY += OVER_SPEED_DECEL;
            }
        ...
        deceleratingSpeed(racer._speed.speedX, std::abs(_speedAccelerate * racer._speed.speedX / 14));
        deceleratingSpeed(racer._speed.speedY, std::abs(_speedAccelerate * racer._speed.speedY / 14));
    } else {
